...
##
@parallel function compute_sigma(dt,dx,dz,C11,C13,C15,C33,C35,C55,beta,v1,v1_3_2_end,
    v3,v3_1_2_end,
    sigmas11,sigmas13,sigmas33,p)

    @inn(sigmas11)=dt*.5*((@all(C11)-@all(C13)) .*@d_xi(v1)/dx+
    (@all(C15)-@all(C35)) .*@d_yi(v1_3_2_end)/dz+
    (@all(C13)-@all(C33)) .*@d_yi(v3)/dz+
    (@all(C15)-@all(C35)) .*@d_xi(v3_1_2_end)/dx)+
    @inn(sigmas11)-
    dt*@all(beta) .*@inn(sigmas11);

    @inn(sigmas33)=dt*.5*((@all(C33)-@all(C13)) .*@d_yi(v3)/dz+
    (@all(C35)-@all(C15)) .*@d_xi(v3_1_2_end)/dx+
    (@all(C13)-@all(C11)) .*@d_xi(v1)/dx+
    (@all(C35)-@all(C15)) .*@d_yi(v1_3_2_end)/dz)+
    @inn(sigmas33)-
    dt*@all(beta).*@inn(sigmas33);

    @inn(sigmas13)=dt*(@all(C55) .*(@d_yi(v1_3_2_end)/dz+
    @d_xi(v3_1_2_end)/dx)+
    @all(C15) .*@d_xi(v1)/dx+
    @all(C35) .*@d_yi(v3)/dz)+
    @inn(sigmas13)-
    ...
    # p
    @inn(p)=-dt*((@all(C11)+@all(C13))*.5 .*@d_xi(v1)/dx+
    (@all(C15)+@all(C35))*.5 .*@d_yi(v1_3_2_end)/dz+
    (@all(C13)+@all(C33))*.5 .*@d_yi(v3)/dz+
    (@all(C15)+@all(C35))*.5 .*@d_xi(v3_1_2_end)/dx)+
    @inn(p)-
    dt*@all(beta).*@inn(p);
...
@timeit ti "mono_2D" function mono_2D(dt,dx,dz,nt,
nx,nz,X,Z,r1,r3,s1,s3,src1,src3,source_type,
r1t,r3t,
Rm,
s1t,s3t,
...
if path_model!=nothing
    ...
    vtkfile = vtk_grid(string(path_model,"/C33"),X,Z);
    vtkfile["C33"]=C.C33;
    vtk_save(vtkfile);
...
for l=1:nt-1
    ...
    @timeit ti "compute_sigma" @parallel compute_sigma(dt,dx,dz,C.C11,C.C13,C.C15,C.C33,C.C35,C.C55,beta,v1,v1_3_2_end,
    v3,v3_1_2_end,
    sigmas11,sigmas13,sigmas33,p);
